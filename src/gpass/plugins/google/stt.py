# ruff: noqa: C901, E501, PLR0913, PLR0915
# LiveKit's API requires many params; some rules disabled to maintain compatibility.
...
class STTOptions:
    ...
    denoiser_config: cloud_speech.DenoiserConfig | None
...
class STT(stt.STT):
    ...
    def __init__(
        ...
        *,
        ...
        denoiser_config: cloud_speech.DenoiserConfig | None = None,
    ):
        ...
        super().__init__(
            capabilities=stt.STTCapabilities(
                streaming=use_streaming, interim_results=True
            )
        )
    ...
    async def _create_client(self, _: float) -> SpeechAsyncClient:
        ...
        if client is None:
            raise ValueError("Client is not initialized but is required.")
        return client
    ...
    def _get_recognizer(self, client: SpeechAsyncClient) -> str:
        ...
        try:
            project_id = client.transport._credentials.project_id
    ...
    async def _run(self) -> None:
        ...
        async def input_generator(
            ...
            try:
                ...
                logger.exception(
                    "an error occurred while streaming input to google STT: %s", err
                )
            ...
            try:
                async with self._pool.connection(
                    ...
                    try:
                        ...
                        should_stop.set()
                        if (
                            not process_stream_task.done()
                            and not wait_reconnect_task.done()
                        ):
...
def _recognize_response_to_speech_event(
    ...
    if resp.results:
        try:
            start_time = _duration_to_seconds(
                resp.results[0].alternatives[0].words[0].start_offset
            )
            end_time = _duration_to_seconds(
                resp.results[-1].alternatives[0].words[-1].end_offset
            )
        except IndexError:
